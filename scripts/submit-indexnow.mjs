...
const EXPECTED_URL_COUNT = EXPECTED_PUBLICATION_COUNT + 3;
...
const publicationData = JSON.parse(await readFile(dataFile, "utf8"));
...
if (urlList.length !== EXPECTED_URL_COUNT || urlList.length > 10_000) {
	throw new Error(
		`Expected ${EXPECTED_URL_COUNT} URLs within the 10,000 URL limit, found ${urlList.length}.`,
	);
}
